...
pygame.display.set_caption('Simulasi Tsunami - Dengan Prediksi Dampak Infrastruktur')
...
# Data infrastruktur pantai
infrastruktur = [
    {"tipe": "rumah_kayu", "x": garis_pantai + 30, "y": permukaan_air - 30, "lebar": 40, "tinggi": 30, "material": "kayu"},
    {"tipe": "hotel", "x": garis_pantai + 90, "y": permukaan_air - 80, "lebar": 60, "tinggi": 80, "material": "beton_bertulang"},
    {"tipe": "toko", "x": garis_pantai + 170, "y": permukaan_air - 40, "lebar": 50, "tinggi": 40, "material": "beton_sederhana"},
    {"tipe": "pelabuhan", "x": garis_pantai - 20, "y": permukaan_air - 10, "lebar": 30, "tinggi": 10, "material": "baja"}
]
...
# Fungsi untuk menggambar infrastruktur
def gambar_infrastruktur(dampak=None):
    for i, bangunan in enumerate(infrastruktur):
        warna = (100, 100, 100)  # Default abu-abu
        
        # Jika ada informasi dampak, sesuaikan warna berdasarkan tingkat kerusakan
        if dampak and i < len(dampak):
            if dampak[i] == "Kerusakan Parah/Total":
                warna = MERAH
            elif dampak[i] == "Kerusakan Moderat":
                warna = KUNING
            elif dampak[i] == "Kerusakan Minimal":
                warna = HIJAU
        
        # Gambar bangunan
        pygame.draw.rect(screen, warna, (bangunan["x"], bangunan["y"], bangunan["lebar"], bangunan["tinggi"]))
        
        # Gambar jendela untuk bangunan
        if bangunan["tinggi"] > 20:
            for j in range(1, 3):
                for k in range(1, min(5, int(bangunan["tinggi"]/20) + 1)):
                    pygame.draw.rect(screen, (200, 200, 255), 
                                    (bangunan["x"] + j*bangunan["lebar"]/3, 
                                     bangunan["y"] + k*bangunan["tinggi"]/5, 
                                     bangunan["lebar"]/10, bangunan["tinggi"]/10))

# Fungsi tampilkan teks
def tampilkan_teks(text, position, size=20, color=HITAM):
    font = pygame.font.SysFont('Arial', size)
    text_surface = font.render(text, True, color)
    screen.blit(text_surface, position)

# Fungsi untuk menghitung gaya gelombang tsunami
def hitung_gaya_gelombang(tinggi_gelombang, kecepatan, densitas_air=1025):
    """Menghitung gaya yang dihasilkan gelombang tsunami pada struktur"""
    return 0.5 * densitas_air * tinggi_gelombang * (kecepatan**2)

# Fungsi untuk memprediksi kerusakan bangunan
def prediksi_kerusakan_bangunan(gaya_gelombang, tipe_bangunan):
    """Memprediksi tingkat kerusakan bangunan berdasarkan gaya gelombang"""
    ketahanan = {
        "kayu": 5000,
        "beton_sederhana": 15000,
        "beton_bertulang": 30000,
        "baja": 45000
    }
    
    if gaya_gelombang < ketahanan[tipe_bangunan] * 0.3:
        return "Kerusakan Minimal"
    elif gaya_gelombang < ketahanan[tipe_bangunan] * 0.7:
        return "Kerusakan Moderat"
    else:
        return "Kerusakan Parah/Total"

# Fungsi untuk menampilkan prediksi dampak tsunami
def tampilkan_prediksi_dampak(tinggi_gelombang, kecepatan):
    dampak = []
    for bangunan in infrastruktur:
        # Hitung jarak dari garis pantai
        jarak_dari_pantai = bangunan["x"] - garis_pantai
        
        # Faktor reduksi berdasarkan jarak (semakin jauh semakin kecil dampaknya)
        faktor_jarak = max(0.1, 1 - (jarak_dari_pantai / 200))
        
        # Hitung gaya gelombang yang mengenai bangunan
        gaya = hitung_gaya_gelombang(tinggi_gelombang, kecepatan) * faktor_jarak
        
        # Prediksi kerusakan
        kerusakan = prediksi_kerusakan_bangunan(gaya, bangunan["material"])
        dampak.append(kerusakan)
    
    # Tampilkan hasil prediksi
    y_pos = 150
    tampilkan_teks("Prediksi Dampak Tsunami pada Infrastruktur:", (20, y_pos))
    y_pos += 30
    
    for i, (bangunan, kerusakan) in enumerate(zip(infrastruktur, dampak)):
        warna = HITAM
        if kerusakan == "Kerusakan Parah/Total":
            warna = MERAH
        elif kerusakan == "Kerusakan Moderat":
            warna = KUNING
        elif kerusakan == "Kerusakan Minimal":
            warna = HIJAU
            
        tampilkan_teks(f"{i+1}. {bangunan['tipe'].capitalize()}: {kerusakan}", (30, y_pos), color=warna)
        y_pos += 25
...
while running:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                ...
                if tahap > 4:
                    tahap = 1
                show_prediction = False
            elif event.key == pygame.K_p and tahap == 4:
    ...
    if tahap == 1:
        ...
        kecepatan_tsunami = 3.0 + tsunami_progress * 5.0  # Kecepatan meningkat saat mendekati pantai
        ...
        # Gambar infrastruktur dengan dampak tsunami jika sudah mencapai pantai
        if tsunami_progress > 0.7 and not show_prediction:
            dampak_prediksi = tampilkan_prediksi_dampak(amplitude_tsunami, kecepatan_tsunami)
            gambar_infrastruktur(dampak_prediksi)
        elif show_prediction:
            # Tampilkan prediksi dampak tsunami
            dampak_prediksi = tampilkan_prediksi_dampak(amplitude_tsunami, kecepatan_tsunami)
            gambar_infrastruktur(dampak_prediksi)
        else:
            gambar_infrastruktur()
        ...
        tampilkan_teks(f"Kecepatan: {kecepatan_tsunami:.1f} m/s", (20, 80))
        tampilkan_teks("Tekan P untuk melihat prediksi dampak", (20, 110))
        tampilkan_teks("Tekan SPACE untuk kembali ke tahap awal", (20, 140))
